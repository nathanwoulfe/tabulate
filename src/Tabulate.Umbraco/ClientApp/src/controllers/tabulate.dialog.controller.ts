export class TabulateDialogController {
  static controllerName = "Tabulate.Dialog.Controller";

  columnTypes: Array<any> = [];
  hasGeocodedAddress = false;
  addressWatch?: Function;
  ...
  $onInit() {
    ...
    this.$scope.model.renderModel = this.$scope.model.config.columns.map(
      (c) => {
        ...
        return {
          ...
          inputType:
            columnTypeData.value === "string" ? "text" : columnTypeData.value,
    ...
    const linkedColumns = this.$scope.model.config.columns.filter(
      (x) => x.type === "linked"
    );
    ...
    if (linkedColumns.length) {
      this.assetsService
        ...
        .then(() =>
          this.$timeout(() =>
            linkedColumns.forEach((l) => this.bindLinkedColumn(l))
          )
        );
    }
    ...
    if (this.$scope.model.type === "edit") {
      this.hasGeocodedAddress =
        this.$scope.model.data._Address &&
      ...
      this.addressWatch = this.$scope.$watch(
        "model.data.Address",
        (newVal, oldVal) => {
          if (newVal !== oldVal) {
  ...
  bindLinkedColumn(column) {
    ...
    const data = linkedTabulateEditor
      ? linkedEditor.value.data
      : linkedEditor.value;
    ...
    //     datumTokenizer: linkedTabulateEditor
    //       ? Bloodhound.tokenizers.obj.whitespace("_label")
    ...
    // const typeaheadElement = angular.element(
    //   "#typeahead_" + this.safeName(column.displayName)
    // );
    // typeaheadElement
