...
export class TabulateController {
  static controllerName = "Tabulate.Controller";

  basePath: string;
  dialogPath: string;
  viewSize: string;
  rteConfig: Record<string, any>;
  search = "";
  ...
  // TODO => why pageNumber, currentPage and pageIndex?
  pagination?: {
    items: [];
    totalPages?: number;
    search?: string;
  ...
  constructor(
    ...
    private tabulateResource,
    private tabulatePagingService
  ) {
    this.basePath =
      Umbraco.Sys.ServerVariables.umbracoSettings.appPluginsPath + "/Tabulate";
    this.dialogPath =
      $scope.model.config.customView || `${this.basePath}/overlays/dialog.html`;
    this.viewSize = $scope.model.config.customView
      ? $scope.model.config.overlaySize
      : "small";
    ...
    $q.all(promises).then((resp) => {
      ...
      if ($scope.model.config.canAccessSettings) {
        const canAccessSettings =
          $scope.model.config.canAccessSettings.split(",");
        this.hideSettings = !resp[1].userGroups.some((x) =>
          canAccessSettings.includes(x)
        );
      }
  ...
  setSorting() {
    ...
    if (!this.settings.sortOrder) {
      ...
      this.data = this.$filter("orderBy")(
        this.data,
        "_label",
        this.settings.sortOrder === "D"
      );
    }
  ...
  removeColumn(col) {
    ...
    if (this.settings?.columns.length === 1) {
      this.overlayService.confirm({
        confirmMessage:
          "Removing all columns will also delete all stored data. Continue?",
        hideHeader: true,
        submit: () => {
          this.settings = {
            columns: [],
  ...
  addRow() {
    const addOverlay = {
      ...this.getOverlayBase("Add row", "add"),
      ...
      submit: (result) => {
        ...
        let newItem = this.mapsLoaded
          ? this.tabulateResource.geocode(result.data)
          : result.data;
  ...
  editRow(guid) {
    ...
    const editOverlay = {
      ...this.getOverlayBase("Edit row", "edit"),
      data: this.data[idx],
      submit: (model) => {
        ...
        model.recode && this.mapsLoaded
          ? this.tabulateResource.geocode(item)
          : {};

        // send new, old and mappings
        this.tabulateResource.updateMappedEditor(
          item,
          originalValue,
  ...
  getCurrentVariant() {
    return this.editorState.getCurrent().variants.find((v) => v.active);
  }
  ...
  openSettings() {
    ...
    const settingsOverlay = {
      ...
      submit: (model) => {
        ...
        const hasColumnChanges = model.changes.some(
          (x) => x.hasOwnProperty("newName") || x.hasOwnProperty("newType")
        );
        if (hasColumnChanges) {
   ...
   */
  setPaging() {
    this.pagination = this.tabulatePagingService.updatePaging(
      this.data,
    ...
    this.noResults =
      this.pagination?.items.length === 0 && this.data.length ? true : false;
  };

  /** */
  setDataGuids() {
    if (this.data[0]._guid) return;
    ...
    this.notificationsService.info(
      "Tabulate data updated - please save and reload"
    );
  };
  ...
  init() {
    if (
      this.$scope.model.value === undefined ||
      this.$scope.model.value.length === 0
    ) {
      this.$scope.model.value = {
        settings: {
          columns: [],
