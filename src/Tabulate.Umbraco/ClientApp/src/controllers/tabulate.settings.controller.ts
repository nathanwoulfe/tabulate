export class TabulateSettingsController {
  static controllerName = "Tabulate.Settings.Controller";

  importKeys: Array<string> = []; // array of header text from imported csv
  types: Array<any>;
  tabulateEditors: Array<any>;
  targetEditorColumns?: Array<any>;
  watchImportExport?: Function;
  geocoder;
  ...
  constructor(
    ...
    if (
      ...
      this.$scope.model.config.columns.length
    ) {
    ...
    if (this.$scope.model.data) {
      ...
      this.csvSource = this.tabulateResource.JSONtoCSV(
    ...
    this.watchImportExport = $scope.$watch(
      () => this.importExport,
      (newVal) => {
        this.importDisabled =
          newVal ===
          (this.showing === "json" ? this.jsonSource : this.csvSource);
      }
    );
  }
  ...
  changedColumn(columnIndex) {
    this.$scope.model.changes[columnIndex].newName =
      this.$scope.model.config.columns[columnIndex].displayName;
    this.$scope.model.changes[columnIndex].newType =
      this.$scope.model.config.columns[columnIndex].type;
  };
  ...
  geocodeAddresses = (index, geoStr, p) => {
    ...
    this.geocoder.geocode({ address: address[p] }, (results, status) => {
      ...
      if (status === google.maps.GeocoderStatus.OK) {
        ...
        this.setImportAlert(
          "danger",
          `Geocoding failed for address: ${address[p]}`
        );
      }

      // TODO => confirm this is correct
      /* recurse through the data object */
      if (index + 1 < this.$scope.model.data.length) {
    ...
    if (csvToJson.length > 0 && csvToJson.length < 2510) {
      ...
      if (this.importKeys.includes("Address")) {
        ...
        if (window.google.maps) {
          ...
          this.setImportAlert(
            "danger",
            "Google maps API not available - geocoding failed"
          );
        }
      }
      ...
      this.setImportAlert(
        "danger",
        "Import failed - dataset must be between 1 and 250 records"
      );
    }
  };
  ...
  importJson = () => {
    try {
      ...
      this.setImportAlert(
        "danger",
        "Import failed - unable to parse JSON input"
      );
    }
  };
  ...
  sort = () => {
    if (this.$scope.model.data && this.$scope.model.config.sortOrder !== "M") {
      this.$scope.model.data = this.$filter("orderBy")(
        this.$scope.model.data,
        "_label",
        this.$scope.model.config.sortOrder === "D"
      );
    }
    this.$scope.model.configChanged = true;
  };
