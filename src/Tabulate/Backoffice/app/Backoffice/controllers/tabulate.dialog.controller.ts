export class TabulateDialogController {
  ...
  constructor(private $scope, private editorService, private editorState, private tabulateResource, private assetsService, private $timeout) {
  }
  ...
  inputType = type => type === 'string' ? 'text' : type;
  ...
  bindLinkedColumn = column => {
    ...
    const sources: { name: string, source: any, displayKey?: string } = {
      name: 'sources',
